...
const npmInstall = (folder) => {
    return new Promise((resolve, reject) => {
        exec(`cd ${folder} && npm install --prefer-dedupe`, (error, stdout, stderr) => {
        if (error) {
        ...
        console.log(chalk.green("👍 " + folder + " installed"));
        resolve(stdout);
        });
    });
    };
...
const npmFindDupes = (folder) => {
    return new Promise((resolve, reject) => {
        exec(`cd ${folder} && npm find-dupes --install-strategy hoisted --no-fund`, (error, stdout, stderr) => {
        if (error) {
            console.log(chalk.red(`Error: ${error.message}`));
            reject(error);
        }
        ...
        console.log(chalk.green("👍 Ran find-dupes on " + folder + "."));
        resolve(stdout);
        });
    });
    }
